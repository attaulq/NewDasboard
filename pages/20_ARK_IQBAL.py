...
KODE_NIK = "IQ189720"
NAMA_AR = "IQBAL LAZUARDI PRANOTO"
...
with h2:
    st.write(""); st.write("")
    st.markdown("# **Dashboard ARK Performance**")
...
def get_val(q):
    ...
    return 0 if df.empty else (df.iloc[0][0] or 0)
...
nama_ar = df_karyawan.iloc[0]["NAMA_AR"] if not df_karyawan.empty else NAMA_AR
jabatan = df_karyawan.iloc[0]["JABATAN"] if not df_karyawan.empty else "Account Representative"
...
with left:
    ...
    foto_path = os.path.join(assets_path, "144520782-iqbal.jpg")
    if os.path.exists(foto_path):
        st.image(foto_path, width=340)

    avg_p = (p_pu + p_bpu) / 2 if (t_pu + t_bpu) else 0
